...
from distiller_cm5_python.utils.uart_utils import signal_app_start
    ...
    try:
        ...
        if PROVIDER_TYPE == "llama-cpp":
            ...
            if not llama_manager.is_running():
                logger.info(
                    f"Llama-cpp server not detected at {SERVER_URL}. Attempting to start..."
                )
                try:
                    llama_manager.start()  # Start the server
                    started_server = True
                    logger.info(
                        f"Llama-cpp server started successfully by main.py (PID: {llama_manager.get_pid()})"
                    )
        ...
        if llama_manager and started_server:
            logger.info(
                f"Shutting down managed llama-cpp server (PID: {llama_manager.get_pid()})..."
            )
