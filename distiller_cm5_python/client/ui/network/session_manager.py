    ...
    def get_sessions_by_ssid(self, ssid: str) -> list[SetupSession]:
        ...
        with self._lock:
            ...
            for session in self.sessions.values():
                if (
                    session.target_ssid == ssid
                    and not session.is_expired()
                    and session.status in [SessionStatus.CONNECTING, SessionStatus.CONNECTED]
                ):
    ...
    def get_connecting_sessions(self) -> list[SetupSession]:
        ...
        with self._lock:
            return [
                session
                for session in self.sessions.values()
                if (session.status == SessionStatus.CONNECTING and not session.is_expired())
            ]
