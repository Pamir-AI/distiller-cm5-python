...
class LlamaCppServerManager:
    ...
    def __init__(self, server_url: str, model_name: str, health_endpoint: str = "/health"):
        ...
        logger.debug(
            f"LlamaCppServerManager initialized for URL: {self.server_url}, Model: {self.model_name}"
        )
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Invalid server URL format for starting server: {self.server_url}. Error: {e}"
            )
            raise UserVisibleError(
                f"Invalid server URL: {self.server_url}. Expected format like http://127.0.0.1:8000"
            )
        ...
        logger.info(
            f"Waiting up to {LLAMA_CPP_START_WAIT_TIME}s for server to become responsive..."
        )
    ...
    def stop(self) -> bool:
        ...
        if process_to_stop is not None:
            ...
            try:
                ...
                ):  # Handle both psutil & subprocess timeout
                    logger.warning(
                        f"Process {pid_to_stop} did not terminate gracefully, attempting force kill."
                    )
                ...
                logger.info(
                    f"Process {pid_to_stop} not found during termination (already terminated?)."
                )
                self._clear_process_info()
                return True
            except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.pid and not psutil.pid_exists(self.pid):
                    logger.warning(
                        f"Server at {self.server_url} is responsive, but original process (PID: {self.pid}) is gone. Clearing internal process state."
                    )
                    self._clear_process_info()
                ...
                logger.debug(
                    f"Llama-cpp connection check failed at {endpoint}. Status: {response.status_code}"
                )
                return False
    ...
    def is_running(self) -> bool:
        ...
        if self.process and self.process.poll() is None:
            ...
            if self.check_connection():
                ...
                logger.warning(
                    f"Managed process (PID: {self.pid}) exists but server at {self.server_url} is unresponsive."
                )
                return False
        ...
        # An external server might be running at the URL
            ...
            logger.info(
                f"Server is responsive at {self.server_url}, but not managed by this instance (PID: {self.pid} is None or terminated). Clearing internal process state."
            )
            self._clear_process_info()
