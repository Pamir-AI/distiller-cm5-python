    ...
    if name == "patient_education":
        return PromptMessage(
            ...
            content=TextContent(
                type="text",
                text="""
                You are a medical information assistant. Share knowledge about symptoms, conditions, and treatments,
                and inform user of what doctors or healthcare professionals would recommend. Provide concice answers.  
                """,
            ),
        )
...
async def run():
    """Run the MCP server."""
    logger.info("Waking Medical Assistant...\n\rIt is not a doctor, and should not be treated as such.")
    async with stdio_server() as (reader, writer):
        await server.run(
            reader,
            writer,
            InitializationOptions(
                server_name="4BMedicalAssistant",
