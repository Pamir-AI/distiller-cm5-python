...
class Config:
    ...
    def _load_from_env(self):
        ...
        if active_provider == "<missing>" or not self.get("llm_providers", active_provider):
            print(f"Warning: Cannot apply environment overrides. Active provider '{active_provider}' not found or defined in llm_providers.")
        ...
        env_provider_mappings = {
            ...
            "LLM_PROVIDER_TYPE": ["provider_type"], # Overrides the type derived from the key? Maybe not needed.
            "LLM_API_KEY": ["api_key"],
            "LLM_TIMEOUT": ["timeout"],
        ...
        for env_var, provider_config_path in env_provider_mappings.items():
            if env_var in os.environ:
                ...
                full_config_path = ["llm_providers", active_provider] + provider_config_path
        ...
        if mcp_server_script:
            self._set_nested_config(self.config, ["mcp_server", "server_script_path"], mcp_server_script)
                ...
                return value.lower() in ("true", "1", "yes")
                 ...
                 if isinstance(curr_value, list) and len(curr_value) > 0:
                     ...
                     try:
                         ...
                         print(f"Warning: Could not convert env var list items '{value}' to type {element_type} for {config_path}. Keeping as string list.")
                         return [item.strip() for item in value.split(',')] # Fallback to string list
                 else: # Default is empty list or not a list? Treat as string list.
                     return [item.strip() for item in value.split(',')]
            else: # Default to string if type is unknown or str
                return value
        except ValueError:
             print(f"Warning: Could not convert env var value '{value}' to type {target_type} for config path {config_path}. Using string value.")
...
if not active_provider_config or not isinstance(active_provider_config, dict):
     raise ValueError(f"Configuration error: No configuration found for active provider '{active_provider_name}' under 'llm_providers'.")
...
PROVIDER_TYPE = get_active_config("provider_type", active_provider_name) # Default to the key name if not specified
